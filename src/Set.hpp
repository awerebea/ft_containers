...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/02/14 01:43:12 by awerebea          #+#    #+#             */
/*                                                    ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
namespace ft {
	...
	template < class T, class Compare = std::less<T>,
		...
		class RBTree {
			...
			std::pair<t_node*, bool>	insert_node(const value_type& val) {
				...
				while (node != NULL) {
					...
					if (is_equal(val, *node->data)) {
						bind_extreme_nodes();
						return std::make_pair(node, false);
					}
					if (compare(val, *node->data)) {
						node = node->left;
					} else {
						node = node->right;
					}
...
#ifdef __linux__
		Set(reverse_iterator first, reverse_iterator last,
				const key_compare& comp = key_compare(),
				const allocator_type& alloc = allocator_type())
			: alloc(alloc), comp(comp) {
			insert(first, last);
		...
		size_type		erase(const value_type& val) {
			t_node* tmp = (tree.search_node(val));
			if (!tmp) { return 0; }
		...
		void			erase(iterator first, iterator last) {
			value_type val = *first.get_node()->data;
			value_type next_val = *(++first).get_node()->data;
			value_type last_val = *(--last).get_node()->data;
			iterator it;
			while (val != last_val) {
				erase(val);
				val = next_val;
				it = iterator(tree.search_node(next_val));
				next_val = *(++it).get_node()->data;
			}
			erase(val);
		}
		...
		iterator		lower_bound(const key_type& k) {
			for (iterator it = begin(); it != end(); ++it) {
				if (tree.compare(k, *it) || tree.is_equal(k, *it)) {
					return it;
				}
			}
			return end();
...
#endif //FT_CONTAINERS_FT_SET_HPP
